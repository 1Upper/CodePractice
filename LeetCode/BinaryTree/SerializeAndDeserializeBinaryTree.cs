...
namespace LeetCode
{
    public class SerializeAndDeserializeBinaryTree
    {
        ...
        public static string serialize(TreeNode root)
        {
            ...
            Queue<TreeNode> nQueue = new Queue<TreeNode>();
            nQueue.Enqueue(root);
            StringBuilder sb = new StringBuilder();

            while (nQueue.Count > 0)
            {
                int qSize = nQueue.Count;

                var node = nQueue.Dequeue();

                if (node != null)
                {
                    sb.Append(node.val.ToString()).Append(",");
                    nQueue.Enqueue(node.left);
                    nQueue.Enqueue(node.right);
                }
                else
                {
                    sb.Append("null,");
        ...
        public static TreeNode deserialize(string data)
        {
            ...
            // Build nodes and save into queue one by one
            Queue<TreeNode> nQueue = new Queue<TreeNode>();
            for (int i = 0; i < nodes.Length; i++)
            {
                if (string.Compare(nodes[i], "null", true) == 0)
                {
                    nQueue.Enqueue(null);
                }
                else
                {
                    nQueue.Enqueue(new TreeNode(int.Parse(nodes[i])));
                }
            }

            // Build tree from root node
            Queue<TreeNode> treeQueue = new Queue<TreeNode>();
            var root = nQueue.Dequeue();
            ...
            while (treeQueue.Count > 0)
            {
                ...
                if (node != null)
                {
                    node.left = nQueue.Dequeue();
                    node.right = nQueue.Dequeue();
    ...
    [TestFixture]
    class UTSerializeAndDeserializeBinaryTree
    {
        [Test]
